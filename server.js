...
const orderRestaurant = require("./models/Order");
...
app.post('/SendOrder', function(req, res){
  ...
  orderRestaurant.create(order, function(err, mobiles){
    if(err){
      throw err;
    }
    res.json(mobiles);
...
app.get('/getOrder', function(req, res){
  orderRestaurant.find(function(err, order){
    if(err){
      throw err;
    }
    res.json(order)
...
// })
// });
// app.delete('/getOrder/:_id',function(req,res){
//   var query = {_id: req.params._id};
// mangoose.model("Order").remove({id:req.params.id},fucntion(err,delData){
